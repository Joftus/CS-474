'''
Next we will do random forests. Make plots of testing and training accuracies of random forests as a function of the depth (using the best num_trees value for that depth),
with the depth ranging from 1 to 20. Report the best depth and corresponding number of trees. Use

    clf=RandomForestClassifier(bootstrap=True,n_estimators=num_trees,max_ features=‘sqrt’,criterion=‘gini’,max_depth=depth)

The argument max_features=‘sqrt’ results in a random, small subset of features being considered for splits in each tree.
For each depth, use the best number of trees for that particular depth, from the sets
...
plt.title('5C training accuracy vs. max_depth')
...
plt.ylim(0, 1.1)
plt.xticks(range(1, 21))
plt.title('5C testing accuracy vs. max_depth')
plt.xlabel('max_depth')
plt.ylabel('accuracy')
